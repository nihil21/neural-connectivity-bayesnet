...
def plot_signals(
    df: typing.Union[pd.DataFrame, typing.Sequence[pd.DataFrame]],
    title: str,
...
def plot_train_test_signals(
    ...
    for i, col in enumerate(df):
        ...
        x_data = range(len(df))
        axes[id1, id2].plot(
            x_data[: len(df_train)], df_train[col], "C1", label="Training set"
        )
        axes[id1, id2].plot(
            x_data[len(df_train) :], df_test[col], "C2", label="Test set"
        )
        ...
        for x_n in next_nodes:
            ...
            for _ in range(indegree - 1):
                ...
                testable_nodes = sorted(
                    set(current_nodes).difference(added_nodes)
                ...
                with ProcessPoolExecutor(max_workers=n_procs) as executor:
                    ...
                    edge_eval_par = functools.partial(
                        edge_eval, estimator=estimator, model=model, x_n=x_n
                    )
                    scores = dict(
                        zip(testable_nodes, executor.map(edge_eval_par, testable_nodes))
                    )
...
def grid_search(
    indegree_range: typing.Sequence[int],
    current_nodes: typing.Sequence[str],
    next_nodes: typing.Sequence[str],
    estimator: BDeuScore,
) -> int:
    ...
    for indegree in indegree_range:
        start_time = time.time()
        _, score = forward_stepwise_selection(
            current_nodes, next_nodes, indegree, estimator
        )
        scores[indegree] = score
        print(
            f"Testing indegree={indegree} with score {score:.2f} [{time.time() - start_time:.2f} s]..."
        )
...
def print_edges(model: typing.Union[BayesianModel, DynamicBayesianNetwork]):
    assert isinstance(model, BayesianModel) or isinstance(
    ...
    sorted_groups = sorted(
        [
            (parent, sorted([child for child in children]))
...
def to_dynamic_cpd(
    ...
    get_dynamic_node = (
        lambda node: (stat_to_dyn_map[node], 0)
        if node.endswith("_T")
    ...
    cpds_info = [
        {
            "variable": get_dynamic_node(cpd.variable),
            "variable_card": 4,
...
def get_order(
    ...
    order = infer._get_elimination_order(
        variables=variables,
        evidence=evidence,
...
def compare_heuristics(
    ...
    with ProcessPoolExecutor(max_workers=n_procs) as executor:
        ...
        get_order_par = functools.partial(
            get_order, evidence=evidence, variables=variables, infer=infer
        )
        orders = dict(zip(heuristics, executor.map(get_order_par, heuristics)))

    if (
        orders["MinFill"][0] == orders["MinNeighbors"][0]
        and orders["MinFill"][0] == orders["MinWeight"][0]
        and orders["MinFill"][0] == orders["WeightedMinFill"][0]
    ):
        if evidence is None:
            print(
                f"All heuristics have found the same elimination order for variables {variables} "
                f"with no evidence:"
            )
        else:
            print(
        ...
        if evidence is None:
            print(
                f"The heuristics have found different elimination orders for variables {variables} "
                f"with no evidence:"
            )
        else:
            print(
...
def generate_time_series(
    ...
    df_synth = df_synth.append(
        pd.Series(next_slice, index=df_synth.columns), ignore_index=True
    )
